...
let CCX: Matrix.Matrix =
    let (first6Rows, last2Rows) = 8 |> Matrix.identity |> Seq.splitAt 6
...
let CSWAP: Matrix.Matrix =
    let (first5Rows, last3Rows) = 8 |> Matrix.identity |> Seq.splitAt 5
    let (lastRow, reversedButLastRows) = last3Rows |> Seq.rev |> Seq.splitAt 1
...
let matrixOfOrderedIndices (numOfQubits: int)
                           (minIndex: int)
                           (middleIndex: int)
                           (maxIndex: int)
    ...
    let sideBySideMatrixA =
        Gate2.matrixOfGettingSideBySide numOfQubits minIndex distanceA

    let sideBySideMatrixB =
        Gate2.matrixOfGettingSideBySide numOfQubits (minIndex + 1) (distanceB - 1)

    let sideBySideMatrix =
        Matrix.standardProduct sideBySideMatrixB sideBySideMatrixA
...
let matrix (numOfQubits: int) (indexA: int) (indexB: int) (indexC: int) (q3Gate: Matrix.Matrix): Matrix.Matrix =
    let indexedIndices =
