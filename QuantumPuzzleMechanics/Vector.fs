...
let maybeThrowException (v1: Vector) (v2: Vector): unit =
    if LazyList.length v1 <> LazyList.length v2 then raise DifferentDimension else ()

// basic converters

let toString (v: Vector): string =
    let componentString = LazyList.map Complex.toString v
...
let zero (n: int): Vector =
    Complex.zero |> LazyList.repeat |> LazyList.take n

// methods

let timesNumber (x: Number.Number) (v: Vector): Vector = LazyList.map (Complex.timesNumber x) v

let timesComplex (c: Complex.Complex) (v: Vector): Vector = LazyList.map (Complex.product c) v

let dim (v: Vector): int = LazyList.length v

let norm (v: Vector): Number.Number =
    v
    |> LazyList.map (fun z -> Complex.absVal z * Complex.absVal z)
    |> LazyList.fold (+) 0.0
    |> sqrt

let opposite (v: Vector): Vector = LazyList.map Complex.opposite v

let unary (v: Vector): Vector option =
    let nrm = norm v
    if nrm = 0.0 then None else timesNumber (1.0 / nrm) v |> Some
...
let innerProduct (v1: Vector) (v2: Vector): Complex.Complex =
    ...
    LazyList.zip v1 v2
    |> LazyList.map (fun (c, z) -> z |> Complex.conjugate |> Complex.product c)
