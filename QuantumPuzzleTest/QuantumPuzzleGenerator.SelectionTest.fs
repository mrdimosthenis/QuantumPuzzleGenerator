...
let ``Select four gates and the quantum states of one qubit with  0_1 difference threshold`` () =
    let (selectedGates, selectedQStates) = selectGatesAndQStates random 1 4 1 0.1
...
let ``Select four gates and the quantum states of one qubit with  0_2 difference threshold`` () =
    let (selectedGates, selectedQStates) = selectGatesAndQStates random 1 4 1 0.2
    (LazyList.length selectedGates, LazyList.length selectedQStates)
    |> should equal (4, 16)
...
[<Fact>]
let ``Select three single-qubit-gates for one qubit with 0_01 difference threshold`` () =
    let randomInstance = System.Random(1000)
    selectGatesAndQStates randomInstance 1 3 1 0.01
    |> fst
    |> LazyList.toList
    |> should equal
        [ Quantum.Gate.TGate 0
          Quantum.Gate.XGate 0
          Quantum.Gate.HGate 0 ]

[<Fact>]
let ``Select four single-qubit-gates for one qubit with 0_2 difference threshold`` () =
    let randomInstance = System.Random(1000)
    selectGatesAndQStates randomInstance 1 4 1 0.2
    |> fst
    |> LazyList.toList
    |> should equal
...
let ``Select 10 single-qubit-gates for four qubits with 0_2 difference threshold`` () =
    ...
    |> should equal
        [ Quantum.Gate.ZGate 0
          ...
          Quantum.Gate.TGate 2
          Quantum.Gate.YGate 0
          Quantum.Gate.YGate 3
          Quantum.Gate.XGate 1
          Quantum.Gate.TGate 3
          Quantum.Gate.HGate 0 ]
